...
public class EventForTooltips
{
	...
	public static void OnItemTooltip(ItemTooltipEvent event)
	{
		...
		else if (event.getItemStack().getItem().equals(Items.POISONOUS_POTATO))
		{
			if (event.getItemStack().getTag() != null && event.getItemStack().getTag().contains("tog_message_for_tc"))
			{
				...
				event.getToolTip().add(new TranslatableComponent("tearsofgaia.message.tinkersconstruct").withStyle(Style.EMPTY.withColor(color)));
				event.getToolTip().add(new TranslatableComponent("tearsofgaia.message.line2").withStyle(Style.EMPTY.withColor(color)));
			}
		}
	}
	...
	private static void AddSpecialTextShownAboveSubtitle(ItemTooltipEvent event, String element, int level)
	{
		...
		{
			event.getToolTip().add(new TranslatableComponent("item.minecraft.totem_of_undying").withStyle(ChatFormatting.YELLOW));
		}
