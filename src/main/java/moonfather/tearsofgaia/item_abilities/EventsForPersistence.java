...
public class EventsForPersistence
{
	...
	private static void DoItemFloat(ItemEntity entity)
	{
		...
		else
		{
			...
			if (entity.getY() >= 9.9 && entity.getDeltaMovement().y() == zeroHeightFloatVelocity)
			{
	...
	public static void DoWorldTickForAir(TickEvent.WorldTickEvent event)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (e.level == event.world)
			{
				...
				else if (e.getY() < 0 || e.getDeltaMovement().y() == zeroHeightFloatVelocity || e.getDeltaMovement().y() == -zeroHeightFloatVelocity || e.getTags().contains("DoItemFloat"))
				{
	...
	private static void DoStupidParticles(Entity entity)
	{
		...
		if (entity.level instanceof ServerWorld)
		{
			...
			for (int i = 1; i <= 8; i++)
			{
				...
				((ServerWorld)entity.level).sendParticles(ParticleTypes.INSTANT_EFFECT,entity.getX() + xSpeed * 8, entity.getY() + ySpeed * 6, entity.getZ() + zSpeed * 8, 1 /*numberOfParticles*/, xSpeed, ySpeed, zSpeed  , 0.10d /*particleSpeed*/ /*, int... particleArguments*/);
		...
		if (!event.getWorld().isClientSide && event.getEntity() instanceof ItemEntity)
		{
				...
				|| ElementalHelper.IsItemElementEqual(stack,"air"))
			{
				...
				if (event.getEntity().getY() < 0)
				{
					...
					if (event.getEntity().getY() < -20)
					{
						//System.out.println("void?");
						event.getEntity().setPos(event.getEntity().getX(), -19, event.getEntity().getZ());
					}
				}
			}
